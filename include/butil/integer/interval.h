...
#include <butil/integer/detail/div.h>
 ...
 *
 * Boundaries are stored as intmax_t or uintmax_t, according Type sign.
 *
...
struct interval
{
    ...
    using boundary_type = int_max_t<Type>;

    static constexpr boundary_type min = Min;
    static constexpr boundary_type max = Max;
};

/** \brief Degenerate interval contains only single value.
 */
template<typename I>
using interval_is_degenerate = std::integral_constant<bool,
...
/** \brief Convert any intset to a corresponding interval.
 */
template<typename I, typename B = typename intset_traits<I>::value_type>
using intset_interval
    = interval<B, intset_traits<I>::min, intset_traits<I>::max>;
...
using interval_equal = std::integral_constant<bool,
      cmp::int_eq(intset_traits<A>::min, intset_traits<B>::min) &&
      cmp::int_eq(intset_traits<A>::max, intset_traits<B>::max)>;

/** \brief Interval boundaries are not equal.
 */
template<typename A, typename B>
using interval_not_equal
    = std::integral_constant<bool, !interval_equal<A, B>::value>;
...
struct interval_meet_impl<A, B>
{
    ...
    using type = interval<common,
            integral_max<common,
                    int_saturate_cast<common>(A::min),
                    int_saturate_cast<common>(B::min)
                >::value,
            integral_min<common,
                    int_saturate_cast<common>(A::max),
                    int_saturate_cast<common>(B::max)
                >::value>;
};

template<typename A, typename B, typename C, typename ... Args>
struct interval_meet_impl<A, B, C, Args...>
    : interval_meet_impl<
        typename interval_meet_impl<A, B>::type, C, Args...>
{};
...
using interval_includes = std::integral_constant<bool,
    cmp::int_le(intset_traits<A>::min, intset_traits<B>::min) &&
    cmp::int_le(intset_traits<B>::max, intset_traits<A>::max)>;
...
using interval_meet_empty = std::integral_constant<bool,
      ...
      cmp::int_lt(intset_traits<B>::max, intset_traits<A>::min)>;
...
{
    return cmp::le(interval_minimum<I>(), x, interval_maximum<I>());
}
...
template<typename A>
struct interval_add_impl<A>
{
    using type = A;
};

template<typename A, typename ... Args>
struct interval_add_impl<A, Args...>
{
    using B = typename interval_add_impl<Args...>::type;
    // prefer uintmax_t
...
struct interval_abs_impl
{
    ...
    static_assert(type::min >= 0, "Absolute value must be nonnegative!");
};
...
template<typename A, typename B, mod_sign S>
struct interval_div_mod_impl
{
    // prefer uintmax_t
    using common = typename intset_type<interval_join<A,B>, prefer_unsigned>::max;

    static_assert(!interval_contains<B>(0), "Possible division by zero");
    //TODO : relax this
    static_assert(intset_greater_equal<A, std::integral_constant<int, 0>>::value, "");
    static_assert(intset_greater<B, std::integral_constant<int, 0>>::value, "");

    using div = interval<common,
          integral_div<common, A::min, B::max, S>::value,
          integral_div<common, B::min, A::max, S>::value>;

    using mod = interval<common, 0, B::max>;
};

}//namespace detail

template<typename A, typename B, mod_sign S>
using interval_div = typename detail::interval_div_mod_impl<intset_interval<A>, intset_interval<B>, S>::div;

template<typename A, typename B, mod_sign S>
using interval_mod = typename detail::interval_div_mod_impl<intset_interval<A>, intset_interval<B>, S>::mod;
