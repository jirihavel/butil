#ifndef BUTIL_INTEGER_DETAIL_DIV_H_INCLUDED
#define BUTIL_INTEGER_DETAIL_DIV_H_INCLUDED
...
/** \brief 3 possible ways of signed division.
 *
 * num = den*quot + rem holds for all of them
 *
 * positive :
 * - Euler division
 * - remainder is always positive
 * dividend :
 * - remainder sign matches sign of dividend
 * - -5/ 2 : -5 = ( 2)*(-2) -1
 *    5/-2 :  5 = (-2)*(-2) +1
 * - usual native division - truncate
 * divisor :
 * - remainder sign matches sign of divisor
 */
enum class mod_sign
{
    positive = 0x0,
...
template<typename X>
constexpr std::pair<X,X> int_divmod(X n, X d, mod_sign s) noexcept
{
...
template<typename X>
constexpr X int_div(X n, X d, mod_sign s) noexcept
{
    return int_divmod(n,d,s).first;
}

template<typename X>
constexpr X int_mod(X n, X d, mod_sign s) noexcept
{
    return int_divmod(n,d,s).second;
}
...
struct integral_div_base
    : std::integral_constant<X, int_div(A, B, S)>
{
};
...
template<typename X, X A, X B, mod_sign S>
struct integral_mod
    : std::integral_constant<X, int_mod(A, B, S)>
{
    static constexpr bool negative_overflow = false;
...
#endif//BUTIL_INTEGER_DETAIL_DIV_H_INCLUDED
